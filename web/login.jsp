...
<html>
    ...
    <body>
         <c:if test='${param.email != null && param.password !=null}'>
            ...
            <sql:query dataSource="${ds}" var="rs" sql="select * from client where email='${param.email}' and password='${param.password}'"/>
            
            <c:if test="${rs.rowCount != 0}">
                <c:if test='${rs.rows[0].password == param.password}'>
                    <c:set var="email" value="${param.email}" scope="session"/>
                    <c:set var="c_id" value="${rs.rows[0].id}" scope="session"/>
                    
                    <c:choose>
                        <c:when test="${param.type_id !=null}">
